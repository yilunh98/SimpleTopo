  ...
  """
    ...
    def act_like_switch(self, packet, packet_in):
        ...
        if packet.dst in self.mac_to_port:
            ...
            print(msg.match._in_port)
            msg.match.dl_src = packet.src
            msg.match.dl_dst = packet.dst
            msg.actions.append(of.ofp_action_output(port=self.mac_to_port[packet.dst]))
            print("actions!!!:",msg.actions)
            msg.idle_timeout = 60
    ...
    """
        trecv = time.time() *1000
        # T1 = (trecv - send_time_1) / 2
        # T2 = (trecv - send_time_2) / 2
        ...
        tsend = packet.find("ethernet")
        tsend = tsend.payload
        #print("something!!!!!!!!!!!",tsend)
        #tsend, = struct.unpack("!d", tsend)
        #print(tsend)
        # delay = trecv - tsend #- T1 - T2
        # print("Path delay from s1(%s) to s2(%s): %f ms" % (packet.src, packet.dst, delay))
...
def _timer_func():
    ...
    # log.debug("Sent %i flow/port stats request(s) at time %s"%(len(core.openflow._connections),send_time))
...
def _handle_portstats_received(event):
    ...
    # echo_latency = (recv_time - send_time) / 2
    # print("echo latency is ", echo_latency)
